...
namespace GuitarEffects
{
    class NAudioEngine : INotifyPropertyChanged, ISpectrumPlayer, IWaveformPlayer, IDisposable
    {
        ...
        private WaveChannel32 pendingWaveFormStream;
        private WaveChannel32 waveFormStream;
        ...
        private WaveChannel32 outputStream;
        ...
        {
            public WaveformGenerationParams(int points, WaveChannel32 effectWave)
            {
                Points = points;
            ...
            public WaveChannel32 EffectWave { get; protected set; }
        }

        private void GenerateWaveformData(WaveChannel32 effectWave)
        {
            if (waveformGenerateWorker.IsBusy)
            {
                pendingWaveFormStream = effectWave;
                waveformGenerateWorker.CancelAsync();
                return;
            }

            if (!waveformGenerateWorker.IsBusy && waveformCompressedPointCount != 0)
                waveformGenerateWorker.RunWorkerAsync(new WaveformGenerationParams(waveformCompressedPointCount, effectWave));
        }
        ...
        private void waveformGenerateWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                if (!waveformGenerateWorker.IsBusy && waveformCompressedPointCount != 0)
                    waveformGenerateWorker.RunWorkerAsync(new WaveformGenerationParams(waveformCompressedPointCount, pendingWaveFormStream));
            }
        }
        ...
        private void waveformGenerateWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            WaveChannel32 waveformInputStream = waveformParams.EffectWave;
        ...
        public void AddCompressors(int thresh, double gain, int gate)
        {
            if(inputStream != null)
            {
                for (int i = 0; i < inputStream.WaveFormat.Channels; i++) this.Effects.Add(new Effects.Compressor(thresh, gain, gate));
            }
        }

        public void AddReverbs(float decay, int delay)
        {
            if (inputStream != null)
            {
                for (int i = 0; i < inputStream.WaveFormat.Channels; i++) this.Effects.Add(new Effects.Reverb(delay, decay));
            }
        }
        ...
        public void ChangeEffects()
        {
            ...
            if (inputStream == null)
            {
                throw new NullReferenceException("InputStream is null,, can't apply effects.");
            }

            effectStream = new EffectStream(inputStream);
            if( Effects.Count % inputStream.WaveFormat.Channels != 0){
                throw new FormatException("Effect and track channels don't match");
            }
            for (int i = 0; i < this.Effects.Count; i++)
            {
                effectStream.Effects.Add(this.Effects[i]);
                Console.Write(this.Effects[i].ToString() + "\n");
            }
            outputStream = new WaveChannel32(effectStream);
            waveFormStream = new WaveChannel32(effectStream);
        ...
        private void outputStream_Sample(object sender, SampleEventArgs e)
        {
